...
improvements, and hook fixes. PRs that don't fit the framework's direction may
be closed with a brief rationale.
...
**Skill files**
- Skills live in `codex-studio/skills/<name>/SKILL.md` — the subdirectory format is
...
- Reasoning tier: `Fast` for read-only status checks, `Deep` for multi-document
  synthesis and phase gates, `Standard` for everything else
...
- Maintainers may enable CODEOWNERS by uncommenting `.github/CODEOWNERS`
