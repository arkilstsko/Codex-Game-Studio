...
description: "Codex Game Studio workflow `/setup-engine`. Use when the user asks for CCGS game-studio workflow `ccgs-setup-engine` or the original `/setup-engine` command. Configure the project's game engine and version. Pins the engine in AGENTS.md, detects knowledge gaps, and populates engine reference docs via Web search when the version is beyond the LLM's training data."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `setup-engine`.
...
- Read `design/gdd/game-concept.md` if it exists — extract genre, scope, platform
  targets, art style, team size, and any engine recommendation from `$ccgs-brainstorm`
...
**Question 1 — Prior experience** (ask this first, always, via `request_user_input`):
...
**Question 2 — Target platform** (ask this second, always, via `request_user_input` — platform eliminates or heavily weights engines before any other factor):
...
5. Use `request_user_input` to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use `request_user_input` with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
### Remaining Sections
- **Performance Budgets**: Use `request_user_input`:
...
Use Web search and Web fetch to locate the official migration guide between
