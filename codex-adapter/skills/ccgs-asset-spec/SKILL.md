...
description: "Codex Game Studio workflow `/asset-spec`. Use when the user asks for CCGS game-studio workflow `ccgs-asset-spec` or the original `/asset-spec` command. Generate per-asset visual specifications and AI generation prompts from GDDs, level docs, or character profiles. Produces structured spec files and updates the master asset manifest. Run after art bible and GDD/level design are approved, before production begins."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `asset-spec`.
...
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and use `request_user_input`:
...
Present the full proposed inventory to the user in conversation. Then use `request_user_input`:
...
- **Technical preferences**: Read `~/.codex/skills/ccgs-references/references/docs/technical-preferences.md` — extract performance budgets and naming conventions.
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use `request_user_input`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use `request_user_input`:
...
Present the full identified list to the user. Use `request_user_input`:
...
Spawn specialist agents based on review mode. **Issue all role-reference passes simultaneously — do not wait for one before starting the next.**

### Full mode — spawn in parallel:

**`art-director`** using the role-reference workflow:
...
**`technical-artist`** using the role-reference workflow:
...
After presenting all specs, use `request_user_input`:
