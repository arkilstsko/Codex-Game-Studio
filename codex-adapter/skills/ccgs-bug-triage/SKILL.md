...
description: "Codex Game Studio workflow `/bug-triage`. Use when the user asks for CCGS game-studio workflow `ccgs-bug-triage` or the original `/bug-triage` command. Read all open bugs in production/qa/bugs/, re-evaluate priority vs. severity, assign to sprints, surface systemic trends, and produce a triage report. Run at sprint start or when the bug count grows enough to need re-prioritization."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `bug-triage`.
