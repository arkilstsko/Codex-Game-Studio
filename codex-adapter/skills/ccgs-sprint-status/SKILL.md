...
description: "Codex Game Studio workflow `/sprint-status`. Use when the user asks for CCGS game-studio workflow `ccgs-sprint-status` or the original `/sprint-status` command. Fast sprint status check. Reads the current sprint plan, scans story files for status, and produces a concise progress snapshot with burndown assessment and emerging risks. Run at any time during a sprint for quick situational awareness. Use when user asks 'how is the sprint going', 'sprint update', 'show sprint progress'."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `sprint-status`.
