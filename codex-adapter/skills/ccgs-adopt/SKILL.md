---
name: ccgs-adopt
description: "Codex Game Studio workflow `/adopt`. Use when the user asks for CCGS game-studio workflow `ccgs-adopt` or the original `/adopt` command. Brownfield onboarding — audits existing project artifacts for template format compliance (not just existence), classifies gaps by impact, and produces a numbered migration plan. Run this when joining an in-progress project or upgrading from an older template version. Distinct from /project-stage-detect (which checks what exists) — this checks whether what exists will actually work with the template's skills."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `adopt`.
...
If the project appears fresh (no artifacts at all), use `request_user_input`:
...
5. Run `$ccgs-gate-check [phase]` → writes `stage.txt` authoritatively
...
and offer to handle it immediately using `request_user_input`. Choose the first
branch that applies:
