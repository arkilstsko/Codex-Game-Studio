...
description: "Codex Game Studio workflow `/code-review`. Use when the user asks for CCGS game-studio workflow `ccgs-code-review` or the original `/code-review` command. Performs an architectural and quality code review on a specified file or set of files. Checks for coding standard compliance, architectural pattern adherence, SOLID principles, testability, and performance concerns."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `code-review`.
...
Spawn all applicable specialists simultaneously using the role-reference workflow — do not wait for one before starting the next.
...
For Logic and Integration stories, also load the `qa-tester` role reference and perform that role pass in parallel with the engine specialists. Pass:
