...
description: "Codex Game Studio workflow `/review-all-gdds`. Use when the user asks for CCGS game-studio workflow `ccgs-review-all-gdds` or the original `/review-all-gdds` command. Holistic cross-GDD consistency and game design review. Reads all system GDDs simultaneously and checks for contradictions between them, stale references, ownership conflicts, formula incompatibilities, and game design theory violations (dominant strategies, economic imbalance, cognitive overload, pillar drift). Run after all MVP GDDs are written, before architecture begins."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `review-all-gdds`.
...
Use `request_user_input` for write permission:
- Prompt: "May I write this review to `design/gdd/gdd-cross-review-[date].md`?"
- Options: `[A] Yes — write the report` / `[B] No — skip`

If any GDDs are flagged for revision, use a second `request_user_input`:
...
After all file writes are complete, use `request_user_input` for a closing widget.
