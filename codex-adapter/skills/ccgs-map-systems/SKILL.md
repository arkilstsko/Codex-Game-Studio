...
description: "Codex Game Studio workflow `/map-systems`. Use when the user asks for CCGS game-studio workflow `ccgs-map-systems` or the original `/map-systems` command. Decompose a game concept into individual systems, map dependencies, prioritize design order, and create the systems index."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `map-systems`.
...
- Read `design/gdd/game-concept.md` — **fail with a clear message if missing**:
  > "No game concept found at `design/gdd/game-concept.md`. Run `$ccgs-brainstorm` first
...
After `$ccgs-design-system` completes, use `request_user_input`:
