...
description: "Codex Game Studio workflow `/team-release`. Use when the user asks for CCGS game-studio workflow `ccgs-team-release` or the original `/team-release` command. Orchestrate the release team: coordinates release-manager, qa-lead, devops-engineer, and producer to execute a release from candidate to deployment."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `team-release`.
...
2. If a version is found: report "No version argument provided — inferred [version] from milestone data. Proceeding." Then confirm with `request_user_input`: "Releasing [version]. Is this correct?"
3. If no version is discoverable: use `request_user_input` to ask "What version number should be released? (e.g., v1.0.0)" and wait for user input before proceeding. Do NOT default to a hardcoded version string.
...
- `role: security-engineer` — Security audit for online/multiplayer/data features
- `role: analytics-engineer` — Telemetry event verification and dashboard readiness
- `role: community-manager` — Patch notes and launch communication
- `role: producer` — Go/no-go decision, stakeholder communication
- `role: network-programmer` — Netcode stability sign-off (invoke if game has multiplayer)
...
delegated to role passes and sub-skills. Each enforces the "May I write to [path]?"
