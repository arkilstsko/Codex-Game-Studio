...
description: "Codex Game Studio workflow `/story-readiness`. Use when the user asks for CCGS game-studio workflow `ccgs-story-readiness` or the original `/story-readiness` command. Validate that a story file is implementation-ready. Checks for embedded GDD requirements, ADR references, engine notes, clear acceptance criteria, and no open design questions. Produces READY / NEEDS WORK / BLOCKED verdict with specific gaps. Use when user says 'is this story ready', 'can I start on this story', 'is story X ready to implement'."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `story-readiness`.
...
**Scope:** `$ARGUMENTS[0]` (blank = ask user via request_user_input)
...
If no argument is given, use `request_user_input`:
...
- **GAPS [list]** → surface the specific gaps to the user via `request_user_input`:
