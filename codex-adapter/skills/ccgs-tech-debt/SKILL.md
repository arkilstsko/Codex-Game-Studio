---
name: ccgs-tech-debt
description: "Codex Game Studio workflow `/tech-debt`. Use when the user asks for CCGS game-studio workflow `ccgs-tech-debt` or the original `/tech-debt` command. Track, categorize, and prioritize technical debt across the codebase. Scans for debt indicators, maintains a debt register, and recommends repayment scheduling."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `tech-debt`.
...
Then use `request_user_input` to collect the **category**:
...
Then use `request_user_input` to collect the **estimated fix effort**:
