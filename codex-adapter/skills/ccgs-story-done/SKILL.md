...
description: "Codex Game Studio workflow `/story-done`. Use when the user asks for CCGS game-studio workflow `ccgs-story-done` or the original `/story-done` command. End-of-story completion review. Reads the story file, verifies each acceptance criterion against the implementation, checks for GDD/ADR deviations, prompts code review, updates story status to Complete, and surfaces the next ready story from the sprint."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `story-done`.
...
If none: flag as **ADVISORY** — "No smoke check report found. Run `$ccgs-smoke-check`."
