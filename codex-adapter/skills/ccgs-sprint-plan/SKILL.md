...
description: "Codex Game Studio workflow `/sprint-plan`. Use when the user asks for CCGS game-studio workflow `ccgs-sprint-plan` or the original `/sprint-plan` command. Generates a new sprint plan or updates an existing one based on the current milestone, completed work, and available capacity. Pulls context from production documents and design backlogs."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `sprint-plan`.
...
```yaml
# Auto-generated by /sprint-plan. Updated by $ccgs-story-done and /dev-story.
...
Before finalising the sprint plan, load the `producer` role reference and perform that role pass using gate **PR-SPRINT** (`~/.codex/skills/ccgs-references/references/docs/director-gates.md`).
...
- `full` — run all director gates as spawned role passes
- `solo` — skip all gates unconditionally (single-developer, no review)

This file is read by `$ccgs-sprint-plan`, `$ccgs-story-readiness`, `$ccgs-story-done`, and other skills at startup.
