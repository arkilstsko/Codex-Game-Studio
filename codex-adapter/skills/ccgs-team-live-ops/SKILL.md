...
description: "Codex Game Studio workflow `/team-live-ops`. Use when the user asks for CCGS game-studio workflow `ccgs-team-live-ops` or the original `/team-live-ops` command. Orchestrate the live-ops team for post-launch content planning: coordinates live-ops-designer, economy-designer, analytics-engineer, community-manager, writer, and narrative-director to design and plan a season, event, or live content update."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `team-live-ops`.
...
Then stop immediately without spawning any role passes or reading any files.
...
- `role: live-ops-designer` — Season/event structure and retention mechanics
- `role: economy-designer` — Live economy balance and reward pricing
- `role: analytics-engineer` — Success metrics, A/B tests, event instrumentation
- `role: community-manager` — Player-facing communication and messaging
- `role: narrative-director` — Seasonal theme and narrative framing
- `role: writer` — All player-facing text: event descriptions, item names, copy
...
delegated to role passes spawned using the role-reference workflow. Each role pass enforces the
