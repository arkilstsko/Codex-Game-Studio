...
description: "Codex Game Studio workflow `/create-stories`. Use when the user asks for CCGS game-studio workflow `ccgs-create-stories` or the original `/create-stories` command. Break a single epic into implementable story files. Reads the epic, its GDD, governing ADRs, and control manifest. Each story embeds its GDD requirement TR-ID, ADR guidance, acceptance criteria, story type, and test evidence path. Run after /create-epics for each epic."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `create-stories`.
...
- If "Skip": leave `## QA Test Cases` with a placeholder: `*Test cases not yet defined — run $ccgs-qa-plan to generate them.*`
...
Use `request_user_input` to close with context-aware next steps:
