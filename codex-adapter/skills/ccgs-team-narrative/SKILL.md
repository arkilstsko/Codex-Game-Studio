...
description: "Codex Game Studio workflow `/team-narrative`. Use when the user asks for CCGS game-studio workflow `ccgs-team-narrative` or the original `/team-narrative` command. Orchestrate the narrative team: coordinates narrative-director, writer, world-builder, and level-designer to create cohesive story content, world lore, and narrative-driven level design."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `team-narrative`.
...
> Usage: `$ccgs-team-narrative [narrative content description]` — describe the story content, scene, or narrative area to work on (e.g., `boss encounter cutscene`, `faction intro dialogue`, `tutorial narrative`). Do not use `request_user_input` here; output the guidance directly.
...
- `role: art-director` — Character visual profiles, environmental visual storytelling, cinematic tone
- `role: level-designer` — Level layouts that serve the narrative, pacing
- `role: localization-lead` — Localization readiness — flags non-localizable strings, cultural assumptions, and i18n gaps
...
Delegate in parallel — issue all three role-reference passes simultaneously before waiting for any result:
...
role passes spawned using the role-reference workflow. Each role pass enforces the "May I write to [path]?"
