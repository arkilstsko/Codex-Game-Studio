...
description: "Codex Game Studio workflow `/team-polish`. Use when the user asks for CCGS game-studio workflow `ccgs-team-polish` or the original `/team-polish` command. Orchestrate the polish team: coordinates performance-analyst, technical-artist, sound-designer, and qa-tester to optimize, polish, and harden a feature or area for release quality."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `team-polish`.
...
> Usage: `$ccgs-team-polish [feature or area]` — specify the feature or area to polish (e.g., `combat`, `main menu`, `inventory system`, `level-1`). Do not use `request_user_input` here; output the guidance directly.
...
- `role: engine-programmer` — Engine-level fixes for rendering, memory, resource loading
...
- `role: tools-programmer` — Content pipeline and editor tool verification
