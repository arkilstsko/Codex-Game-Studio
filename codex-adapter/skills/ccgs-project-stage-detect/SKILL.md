...
description: "Codex Game Studio workflow `/project-stage-detect`. Use when the user asks for CCGS game-studio workflow `ccgs-project-stage-detect` or the original `/project-stage-detect` command. Automatically analyze project state, detect stage, identify gaps, and recommend next steps based on existing artifacts. Use when user asks 'where are we in development', 'what stage are we in', 'full project audit'."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `project-stage-detect`.
...
if it exists, use its value (explicit override from `$ccgs-gate-check`). Otherwise,
...
Use template: `~/.codex/skills/ccgs-references/references/docs/templates/project-stage-report.md`
