...
description: "Codex Game Studio workflow `/security-audit`. Use when the user asks for CCGS game-studio workflow `ccgs-security-audit` or the original `/security-audit` command. Audit the game for security vulnerabilities: save tampering, cheat vectors, network exploits, data exposure, and input validation gaps. Produces a prioritised security report with remediation guidance. Run before any public release or multiplayer launch."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `security-audit`.
...
Read `~/.codex/skills/ccgs-references/references/docs/technical-preferences.md` to determine:
...
Load the `security-engineer` role reference and perform that role pass. Pass:
