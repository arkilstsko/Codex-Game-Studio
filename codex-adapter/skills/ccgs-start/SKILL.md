---
name: ccgs-start
description: "Codex Game Studio workflow `/start`. Use when the user asks for CCGS game-studio workflow `ccgs-start` or the original `/start` command. First-time onboarding — asks where you are, then guides you to the right workflow. No assumptions."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `start`.
...
Check:
- **Engine configured?** Read `~/.codex/skills/ccgs-references/references/docs/technical-preferences.md`. If the Engine field contains `[TO BE CONFIGURED]`, the engine is not set.
...
This is the first thing the user sees. Use `request_user_input` with these exact options so the user can click rather than type:
...
2. Briefly explain what `$ccgs-brainstorm` does (guided ideation using professional frameworks — MDA, player psychology, verb-first design). Mention that it has two modes: `$ccgs-brainstorm open` for fully open exploration, or `$ccgs-brainstorm [hint]` if they have even a vague theme (e.g., "space", "cozy", "horror").
3. Recommend running `$ccgs-brainstorm open` as the next step, but invite them to use a hint if something comes to mind
4. Show the recommended path:
   **Concept phase:**
   - `$ccgs-brainstorm open` — discover your game concept
   - `$ccgs-setup-engine` — configure the engine (brainstorm will recommend one)
   - `$ccgs-prototype` — throwaway concept build: validate the core idea is fun before designing (1–3 days)
   - `$ccgs-art-bible` — define visual identity (uses the Visual Identity Anchor brainstorm produces)
   - `$ccgs-map-systems` — decompose the concept into systems
   - `$ccgs-design-system` — author a GDD for each MVP system
   - `$ccgs-review-all-gdds` — cross-system consistency check
   - `$ccgs-gate-check` — validate readiness before architecture work
   **Architecture phase:**
   - `$ccgs-create-architecture` — produce the master architecture blueprint and Required ADR list
   - `$ccgs-architecture-decision (×N)` — record key technical decisions, following the Required ADR list
   - `$ccgs-create-control-manifest` — compile decisions into an actionable rules sheet
   - `$ccgs-architecture-review` — validate architecture coverage
   **Pre-Production phase:**
   - `$ccgs-ux-design` — author UX specs for key screens (main menu, HUD, core interactions)
   - `$ccgs-vertical-slice` — production-quality end-to-end build to validate the full game loop
   - `$ccgs-playtest-report (×1+)` — document each vertical slice playtest session
   - `$ccgs-create-epics` — map systems to epics
   - `$ccgs-create-stories` — break epics into implementable stories
   - `$ccgs-sprint-plan` — plan the first sprint
   **Production phase:** → pick up stories with `$ccgs-dev-story`
...
4. Show the recommended path:
   **Concept phase:**
   - `$ccgs-brainstorm [hint]` — develop the idea into a full concept
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not request_user_input (it's an open response).
2. Acknowledge the concept, then use `request_user_input` to offer two paths:
...
3. Show the recommended path:
   **Concept phase:**
   - `$ccgs-brainstorm` or `$ccgs-setup-engine` — (their pick from step 2)
   - `$ccgs-prototype` — throwaway concept build: validate the core idea is fun before designing (1–3 days)
   - `$ccgs-art-bible` — define visual identity (after brainstorm if run, or after concept doc exists)
   - `$ccgs-design-review` — validate the concept doc
   - `$ccgs-map-systems` — decompose the concept into individual systems
   - `$ccgs-design-system` — author a GDD for each MVP system
   - `$ccgs-review-all-gdds` — cross-system consistency check
   - `$ccgs-gate-check` — validate readiness before architecture work
   **Architecture phase:**
...
**If it does not exist**: Use `request_user_input`:
...
- **Options**:
  ...
  - `Lean (recommended)` — Directors only at phase gate transitions ($ccgs-gate-check). Skips per-skill reviews. Balanced approach for solo devs and small teams.
...
After presenting the recommended path, use `request_user_input` to ask the user which step they'd like to take first. Never auto-run the next skill.
