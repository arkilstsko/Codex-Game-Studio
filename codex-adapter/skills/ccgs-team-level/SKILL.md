...
description: "Codex Game Studio workflow `/team-level`. Use when the user asks for CCGS game-studio workflow `ccgs-team-level` or the original `/team-level` command. Orchestrate level design team: level-designer + narrative-director + world-builder + art-director + systems-designer + qa-tester for complete area/level creation."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `team-level`.
...
- `role: accessibility-specialist` — Navigation clarity, colorblind safety, cognitive load
...
Spawn all three agents simultaneously — issue all three role-reference passes before waiting for any result.
...
**Gate**: Use `request_user_input` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
...
After all role pass outputs are collected, load the `level-designer` role reference and perform that role pass to compile and write the final document:
- Pass: all role pass outputs (verbatim), the level brief, game pillars, relevant GDD sections
...
- Perform approved file writes in the current Codex thread for the final document.

5. **Save to** `design/levels/[level-name].md` (handled by the level-designer role pass after user approval — see above).
...
to role passes spawned using the role-reference workflow. Each role pass enforces the "May I write to [path]?"
