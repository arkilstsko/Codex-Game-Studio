...
2. `request_user_input` presents the narrative brief; user approves before Phase 2 begins
...
4. `request_user_input` presents world foundation and dialogue drafts; user approves before Phase 3 begins
5. Phase 3: level-designer is spawned; produces environmental storytelling layout, trigger placement, and pacing plan
6. `request_user_input` presents level narrative plan; user approves before Phase 4 begins
...
6. `request_user_input` presents options:
...
5. `request_user_input` presents options:
...
- [ ] `request_user_input` offers the option to fix now vs. flag and proceed
