...
| **Codex** | AI coding agent | Install the Codex app or CLI for your environment |
...
- Git for Windows includes **Git Bash**, which provides the `bash` command
  used by all hooks in `hook-config.json`
- Ensure Git Bash is on your PATH (default if installed via the Git installer)
- Hooks use `bash ~/.codex/skills/ccgs-references/references/hooks/[name].sh` — this works on Windows because
