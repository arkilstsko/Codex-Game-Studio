...
description: "Codex Game Studio workflow `/regression-suite`. Use when the user asks for CCGS game-studio workflow `ccgs-regression-suite` or the original `/regression-suite` command. Map test coverage to GDD critical paths, identify fixed bugs without regression tests, flag coverage drift from new features, and maintain tests/regression-suite.md. Run after implementing a bug fix or before a release gate."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `regression-suite`.
