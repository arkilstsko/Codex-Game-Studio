...
description: "Codex Game Studio workflow `/create-epics`. Use when the user asks for CCGS game-studio workflow `ccgs-create-epics` or the original `/create-epics` command. Translate approved GDDs + architecture into epics — one epic per architectural module. Defines scope, governing ADRs, engine risk, and untraced requirements. Does NOT break into stories — run /create-stories [epic-slug] after each epic is created."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `create-epics`.
...
- `~/.codex/skills/ccgs-references/references/docs/engine-reference/[engine]/VERSION.md` — engine name, version, risk levels
...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, load the `producer` role reference and perform that role pass using gate **PR-EPIC** (`~/.codex/skills/ccgs-references/references/docs/director-gates.md`).
