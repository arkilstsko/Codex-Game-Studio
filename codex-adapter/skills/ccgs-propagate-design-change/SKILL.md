...
description: "Codex Game Studio workflow `/propagate-design-change`. Use when the user asks for CCGS game-studio workflow `ccgs-propagate-design-change` or the original `/propagate-design-change` command. When a GDD is revised, scans all ADRs and the traceability index to identify which architectural decisions are now potentially stale. Produces a change impact report and guides the user through resolution."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `propagate-design-change`.
...
Load the `technical-director` role reference and perform that role pass using gate **TD-CHANGE-IMPACT** (`~/.codex/skills/ccgs-references/references/docs/director-gates.md`).
...
- **CONCERNS** → surface the specific ADRs or recommendations flagged; use `request_user_input` with options: `Revise the impact assessment` / `Accept with noted concerns` / `Discuss further`
