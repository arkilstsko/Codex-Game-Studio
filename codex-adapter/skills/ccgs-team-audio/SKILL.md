...
description: "Codex Game Studio workflow `/team-audio`. Use when the user asks for CCGS game-studio workflow `ccgs-team-audio` or the original `/team-audio` command. Orchestrate audio team: audio-director + sound-designer + technical-artist + gameplay-programmer for full audio pipeline from direction to implementation."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `team-audio`.
...
> Usage: `$ccgs-team-audio [feature or area]` — specify the feature or area to design audio for (e.g., `combat`, `main menu`, `forest biome`, `boss encounter`). Do not use `request_user_input` here; output the guidance directly.
...
Spawn the **primary engine specialist** in parallel (from `~/.codex/skills/ccgs-references/references/docs/technical-preferences.md` Engine Specialists) to validate the integration approach:
...
5. **Save to** `design/audio/audio-[feature].md`.
   ...
   Note: If `design/audio/` does not exist, the role pass writing the document should create it (the directory will be created automatically when the file is written).
