---
name: ccgs-team-qa
description: "Codex Game Studio workflow `/team-qa`. Use when the user asks for CCGS game-studio workflow `ccgs-team-qa` or the original `/team-qa` command. Orchestrate the QA team through a full testing cycle. Coordinates qa-lead (strategy + test plan) and qa-tester (test case writing + bug reporting) to produce a complete QA package for a sprint or feature. Covers: test plan generation, test case writing, smoke check gate, manual QA execution, and sign-off report."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `team-qa`.
...
the user with the role pass's proposals as selectable options. Write the agent's
...
For each listed role, load the role reference and perform that role pass in the current Codex thread. Use Codex role passes only when explicitly available and requested:
- `role: qa-lead` — Strategy, planning, classification, sign-off
- `role: qa-tester` — Test case writing and bug report writing
...
Load the `qa-lead` role reference and perform that role pass to review all in-scope stories and produce a QA strategy.
...
Load the `qa-tester` role reference and perform that role pass for each story (run in parallel where possible), providing:
- The story file path
...
Batch stories into groups of up to 3 and use `request_user_input` for each:
...
After each FAIL result: use `request_user_input` to collect the failure description, then load the `qa-tester` role reference and perform that role pass to write a formal bug report in `production/qa/bugs/`.
...
Load the `qa-lead` role reference and perform that role pass to produce the sign-off report using all results from Phases 4–6.
