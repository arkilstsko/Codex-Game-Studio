...
Use `request_user_input` to confirm proceeding or reduce scope.
...
Load the `release-manager` role reference and perform that role pass. Ask them to produce a rollback plan covering:
...
1. Load the `lead-programmer` role reference and perform that role pass with:
...
3. Load the `qa-tester` role reference and perform that role pass to verify: does the bug reproduce after the fix?
...
Load the `qa-lead` role reference and perform that role pass with:
