...
description: "Codex Game Studio workflow `/quick-design`. Use when the user asks for CCGS game-studio workflow `ccgs-quick-design` or the original `/quick-design` command. Lightweight design spec for small changes — tuning adjustments, minor mechanics, balance tweaks. Skips full GDD authoring when a system GDD already exists or the change is too small to warrant one. Produces a Quick Design Spec that embeds directly into story files."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `quick-design`.
...
Present the inferred classification using `request_user_input`:
...
Present the draft to the user in full. Then use `request_user_input`:
