...
description: "Codex Game Studio workflow `/architecture-review`. Use when the user asks for CCGS game-studio workflow `ccgs-architecture-review` or the original `/architecture-review` command. Validates completeness and consistency of the project architecture against all GDDs. Builds a traceability matrix mapping every GDD technical requirement to ADRs, identifies coverage gaps, detects cross-ADR conflicts, verifies engine compatibility consistency across all decisions, and produces a PASS/CONCERNS/FAIL verdict. The architecture equivalent of /design-review."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `architecture-review`.
...
If [B]: display the complete proposed systems-index section, then re-ask with `request_user_input`.
...
Use `request_user_input` for the write approval:
...
For `rtm` mode, use `request_user_input`:
...
Then close with `request_user_input` tailored to the pre-gate checklist state:
...
3. **Offer options** via request_user_input with three choices:
