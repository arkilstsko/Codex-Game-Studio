...
> "Player journey map not yet created. Template available at `~/.codex/skills/ccgs-references/references/docs/templates/player-journey.md`. Run `$ccgs-ux-design` Phase 2b or create it manually to establish player context for this screen."
...
6. **Approval**: Use `request_user_input`:
...
7. **Write**: Use `request_user_input`: "May I write the [section name] section to `[filepath]`?"
...
- Use `request_user_input` to capture the choice:
...
- Use `request_user_input`: "Want an ASCII wireframe as part of this spec?"
...
> "Accessibility tier not yet defined — consider WCAG-AA as a baseline. Run `$ccgs-gate-check` to see whether this blocks any phase gates."
...
Write at least 5 specific, testable criteria that a QA tester can verify without reading any other design document. These become the pass/fail conditions for `$ccgs-story-done`.
...
Use `request_user_input` to step through items in groups of up to 3, not all at once.
...
Work through patterns in groups. Use `request_user_input`:
...
When delegating to another agent via the Codex role-reference workflow:
