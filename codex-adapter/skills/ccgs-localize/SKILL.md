...
Load the `localization-lead` role reference and perform that role pass. Ask them to audit the following for cultural sensitivity across the target locales (read from `assets/data/strings/` and `assets/`):
...
Load the `localization-lead` role reference and perform that role pass with:
