...
- [ ] Concept prototype exists in `prototypes/` with a REPORT.md showing PROCEED verdict
      (`$ccgs-prototype [core-mechanic]`) — skipping this means GDDs may be written for an
      idea that hasn't been played. Acceptable if the concept is proven by other means.

**Quality Checks:**
- [ ] Game concept has been reviewed (`$ccgs-design-review` verdict not MAJOR REVISION NEEDED)
...
- [ ] All MVP-tier GDDs exist in `design/gdd/` and individually pass `$ccgs-design-review`
- [ ] A cross-GDD review report exists in `design/gdd/` (from `$ccgs-review-all-gdds`)

**Quality Checks:**
- [ ] All MVP GDDs pass individual design review (8 required sections, no MAJOR REVISION NEEDED verdict)
- [ ] `$ccgs-review-all-gdds` verdict is not FAIL (cross-GDD consistency and design theory checks pass)
- [ ] All cross-GDD consistency issues flagged by `$ccgs-review-all-gdds` are resolved or explicitly accepted
...
- [ ] `$ccgs-architecture-review` has been run (a review report file exists in `docs/architecture/`)
...
**Required Artifacts:**
- [ ] Vertical slice exists in `prototypes/` with a REPORT.md (run `$ccgs-vertical-slice`) — **recommended, not blocking**; if absent, surface as CONCERNS
...
- [ ] Entity inventory exists at `design/assets/entity-inventory.md` (recommended — run `$ccgs-asset-spec` with no arguments to generate collaboratively from GDDs + art bible)
...
- [ ] Control manifest exists at `docs/architecture/control-manifest.md`
      (generated by `$ccgs-create-control-manifest` from Accepted ADRs)
- [ ] Epics defined in `production/epics/` with at least Foundation and Core
      layer epics present (use `$ccgs-create-epics layer: foundation` and
      `$ccgs-create-epics layer: core` to create them, then `$ccgs-create-stories [epic-slug]`
      for each epic)
...
- [ ] All key screen UX specs have passed `$ccgs-ux-review` (verdict APPROVED or NEEDS REVISION accepted)
...
- [ ] Manual validation confirms GDDs + architecture + epics are coherent
      (run `$ccgs-review-all-gdds` and `$ccgs-architecture-review` if not done recently)
...
- [ ] QA plan exists in `production/qa/` (generated by `$ccgs-qa-plan`) covering this sprint or final production sprint
- [ ] At least one QA plan exists in `production/qa/` covering this production phase — run `$ccgs-qa-plan` if missing (CONCERNS — advisory, not blocking)
- [ ] QA sign-off report exists in `production/qa/` (generated by `$ccgs-team-qa`) with verdict APPROVED or APPROVED WITH CONDITIONS
...
- [ ] QA test plan exists (`$ccgs-qa-plan` output in `production/qa/`)
- [ ] QA sign-off report exists (`$ccgs-team-qa` output — APPROVED or APPROVED WITH CONDITIONS)
...
- [ ] Balance data has been reviewed (`$ccgs-balance-check` run)
- [ ] Release checklist completed (`$ccgs-release-checklist` or `$ccgs-launch-checklist` run)
...
surface it prominently: "No `$ccgs-review-all-gdds` report found in `design/gdd/`. Run
`$ccgs-review-all-gdds` before advancing to Technical Setup."
...
- For performance checks: `Read` technical-preferences.md and compare against any
  profiling data in `tests/performance/` or recent `$ccgs-perf-profile` output
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using `request_user_input`.
