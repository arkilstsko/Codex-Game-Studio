---
name: ccgs-prototype
description: "Codex Game Studio workflow `/prototype`. Use when the user asks for CCGS game-studio workflow `ccgs-prototype` or the original `/prototype` command. Concept prototype — validate the core idea is worth designing before writing GDDs. Run right after /brainstorm and /setup-engine. Routes to HTML, Engine, or Paper path based on game type. Produces a throwaway build and a PROCEED/PIVOT/KILL verdict."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `prototype`.
...
Otherwise, use `request_user_input` to confirm intent before proceeding:
...
Assess which path best fits the hypothesis, then use `request_user_input` with your
...
- `full` → load the `creative-director` role reference and perform that role pass using gate **CD-PLAYTEST** if
...
2. **Choose path** — same request_user_input widget as Phase 3 (HTML / Engine / Paper).
