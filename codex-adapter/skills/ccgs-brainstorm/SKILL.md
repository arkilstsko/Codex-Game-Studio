...
description: "Codex Game Studio workflow `/brainstorm`. Use when the user asks for CCGS game-studio workflow `ccgs-brainstorm` or the original `/brainstorm` command. Guided game concept ideation — from zero idea to a structured game concept document. Uses professional studio ideation techniques, player psychology frameworks, and structured creative exploration."
---

## Codex Adapter

This is the Codex Game Studio workflow skill `brainstorm`.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an request_user_input with preset options.)*
...
Bundle these into a single multi-step `request_user_input` with these exact tab labels:
...
Present all three. Then use `request_user_input` to capture the selection.
...
Ask these as `request_user_input` calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use `request_user_input`:
...
If the user selects B, C, or D, make the revision, then use `request_user_input` again:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` using the role-reference workflow in parallel before moving to Phase 5. Issue both role-reference passes simultaneously — do not wait for one before starting the other.**

- **`creative-director`** — gate **CD-PILLARS** (`~/.codex/skills/ccgs-references/references/docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`~/.codex/skills/ccgs-references/references/docs/director-gates.md`)
...
Collect both verdicts, then present them together using a two-tab `request_user_input`:
...
- **Target platform**: Use `request_user_input` — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use `request_user_input` — "Do you already have an engine you work in?"
...
**After identifying biggest technical risks, load the `technical-director` role reference and perform that role pass using gate TD-FEASIBILITY (`~/.codex/skills/ccgs-references/references/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, load the `producer` role reference and perform that role pass using gate PR-SCOPE (`~/.codex/skills/ccgs-references/references/docs/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `~/.codex/skills/ccgs-references/references/docs/templates/game-concept.md`. Fill in ALL sections from the
...
5. Use `request_user_input` for write approval:
...
If [B]: ask which section to revise using `request_user_input` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `request_user_input` — "Ready to write the updated concept document?"
Options: `[A] Yes — write it` / `[B] Revise another section`
Repeat until the user selects [A].

If yes, generate the document using the template at `~/.codex/skills/ccgs-references/references/docs/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
