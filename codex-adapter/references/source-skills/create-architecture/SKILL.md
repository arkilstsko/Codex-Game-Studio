...
3. `codex-studio/docs/technical-preferences.md` — naming conventions, performance budgets,
...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then use `request_user_input`:
...
Apply gate **TD-ARCHITECTURE** (`codex-studio/docs/director-gates.md`) as a self-review. Check all four criteria from that gate definition against the completed document.
...
**Step 2 — Load the `lead-programmer` role reference and perform that role pass using gate LP-FEASIBILITY (`codex-studio/docs/director-gates.md`):**
...
Use `request_user_input` — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use `request_user_input`:
...
5. **Use `request_user_input` for write approvals** — plain text "May I?" is not
