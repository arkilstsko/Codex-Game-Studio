...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `request_user_input`:
...
- Read `codex-studio/docs/technical-preferences.md` to identify the engine and version
...
Use the template structure from `codex-studio/docs/templates/game-design-document.md`:
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   `request_user_input`. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use `request_user_input` with a multi-step widget:
...
**Framing question (ask BEFORE drafting)**: Use `request_user_input`:
...
load the `creative-director` role reference and perform that role pass:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents using the role-reference workflow in parallel:
...
- Surface any disagreements between agents to the user via `request_user_input`
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents using the role-reference workflow in parallel:
...
- Present the specialists' proposals to the user for review via `request_user_input`
...
For required systems: **load the `art-director` role reference and perform that role pass** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
Before finalizing the GDD, load the `creative-director` role reference and perform that role pass using gate **CD-GDD-ALIGN** (`codex-studio/docs/director-gates.md`).
...
**When delegating using the role-reference workflow tool**:
