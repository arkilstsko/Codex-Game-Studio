...
- **`creative-director`** — gate **CD-PILLARS** (`codex-studio/docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`codex-studio/docs/director-gates.md`)
...
**After identifying biggest technical risks, load the `technical-director` role reference and perform that role pass using gate TD-FEASIBILITY (`codex-studio/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, load the `producer` role reference and perform that role pass using gate PR-SCOPE (`codex-studio/docs/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `codex-studio/docs/templates/game-concept.md`. Fill in ALL sections from the
...
If yes, generate the document using the template at `codex-studio/docs/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
