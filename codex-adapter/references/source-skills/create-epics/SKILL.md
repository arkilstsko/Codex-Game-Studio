...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, load the `producer` role reference and perform that role pass using gate **PR-EPIC** (`codex-studio/docs/director-gates.md`).
