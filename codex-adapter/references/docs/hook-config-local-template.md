# hook-config.local.json Template

Create `codex-studio/hook-config.local.json` for personal overrides that should NOT
be committed to version control. Add it to `.gitignore`.

## Example hook-config.local.json
...
You can add personal hooks in `hook-config.local.json` that extend (not override)
