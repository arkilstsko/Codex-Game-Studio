...
Templates are in `codex-studio/docs/templates/`:
...
Also in `codex-studio/docs/templates/collaborative-protocols/` (used by agents, not typically edited directly):
...
2. **Set up the engine** — Run `/setup-engine` (uses the brainstorm recommendation)
   ...
   - Creates `codex-studio/docs/technical-preferences.md` with naming conventions,
...
codex-studio/
  hook-config.json                    -- Codex reference hooks and project settings
  agents/                          -- 49 agent definitions (YAML frontmatter)
  skills/                          -- 73 slash command definitions (YAML frontmatter)
  hooks/                           -- 12 hook scripts (.sh) wired by hook-config.json
  ...
  docs/
    ...
    hook-config-local-template.md     -- Personal hook-config.local.json guide
