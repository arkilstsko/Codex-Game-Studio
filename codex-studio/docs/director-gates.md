...
- Gate: [GATE-ID] (see codex-studio/docs/director-gates.md)
...
Spawn all [N] agents simultaneously using the role-reference workflow — issue all role-reference passes before
...
| **CONCERNS [list]** | Issues present but not blocking. | Surface to user via `request_user_input` — options: `Revise flagged items` / `Accept and proceed` / `Discuss further` |
...
- Game pillars and core fantasy
- Platform and performance constraints (from `codex-studio/docs/technical-preferences.md` if configured)
...
```
Spawn in parallel (issue all role-reference passes before waiting for any result):
