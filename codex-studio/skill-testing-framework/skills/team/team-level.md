...
2. Step 1 — narrative-director spawned: defines narrative purpose, key characters, dialogue triggers, emotional arc; world-builder spawned: provides lore context, environmental storytelling opportunities, world rules; `request_user_input` confirms Step 1 outputs before Step 2
3. Step 2 — level-designer spawned: designs spatial layout (critical path, optional paths, secrets), pacing curve, encounters, puzzles, entry/exit points and connections to adjacent areas; `request_user_input` confirms layout before Step 3
...
4. `request_user_input` presented with options:
...
4. Skill presents both Step 4 results via `request_user_input` — BLOCKING concern highlighted prominently
5. `request_user_input` offers:
...
- [ ] `request_user_input` presents the specific concern text (not just "accessibility issue found")
...
- [ ] `request_user_input` offers a "design crystal caves first" option referencing `/team-level`
