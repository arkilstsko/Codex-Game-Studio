...
**After dependency mapping is approved, load the `technical-director` role reference and perform that role pass using gate TD-SYSTEM-BOUNDARY (`codex-studio/docs/director-gates.md`) before proceeding to priority assignment.**
...
**After priorities are approved, load the `producer` role reference and perform that role pass using gate PR-SCOPE (`codex-studio/docs/director-gates.md`) before writing the index.**
...
**After the systems index is written, load the `creative-director` role reference and perform that role pass using gate CD-SYSTEMS (`codex-studio/docs/director-gates.md`).**
...
After the systems index is created (or after designing some systems), present next actions using `request_user_input`:
