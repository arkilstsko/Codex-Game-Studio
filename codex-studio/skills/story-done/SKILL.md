...
batch up to 3 manual verification questions into a single `request_user_input` call:
...
After completing the deviation checks in Phase 4, load the `qa-lead` role reference and perform that role pass using gate **QL-TEST-COVERAGE** (`codex-studio/docs/director-gates.md`).
...
Load the `lead-programmer` role reference and perform that role pass using gate **LP-CODE-REVIEW** (`codex-studio/docs/director-gates.md`).
...
Use `request_user_input` before writing anything:
...
- Use `request_user_input` for the code review prompt and for batching manual
