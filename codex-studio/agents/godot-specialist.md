...
You have access to the Codex role-reference workflow to delegate to your sub-specialists. Use it when a task requires deep expertise in a specific Godot subsystem:

- `role: godot-gdscript-specialist` — GDScript architecture, static typing, signals, coroutines
- `role: godot-shader-specialist` — Godot shading language, visual shaders, particles
- `role: godot-gdextension-specialist` — C++/Rust bindings, native performance, custom nodes
