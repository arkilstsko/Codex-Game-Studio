...
> - `$ccgs-ux-review` verdict: NEEDS REVISION was resolved — current state: APPROVED ✅
>
...
- **UX review is a hard gate**: `$ccgs-team-ui` checks for a passing `$ccgs-ux-review` before proceeding to visual design
...
- **Section-by-section UX authoring**: same incremental pattern as `$ccgs-design-system` — each section written to file before moving on
