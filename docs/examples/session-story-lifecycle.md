...
**Context:** Solo indie dev, mid-production sprint. The movement system GDD and architecture are complete. A story for movement's core implementation has been generated by `$ccgs-create-stories`.
...
**Turns:** 13
**Outcome:** Story validated ready, implemented, verified complete with `$ccgs-story-done`
...
>
> Run `$ccgs-story-readiness` on whichever you pick up next.

---

## What This Example Demonstrates

- **Readiness gate catches real issues**: the roll direction ambiguity would have caused a late implementation decision without `$ccgs-story-readiness`
...
- **Deferred criteria**: not every acceptance criterion can be verified at story close — `$ccgs-story-done` tracks deferred items rather than blocking completion
...
- **sprint-status.yaml**: single source of truth for sprint state, updated by `$ccgs-story-done` and read by `$ccgs-sprint-status`
