...
> | Gap | Severity | Reason |
> |-----|----------|--------|
> | No systems index | BLOCKING | `$ccgs-design-system`, `$ccgs-create-stories`, `$ccgs-gate-check` all require it |
> | GDD files not in template format | HIGH | Skills cannot reference TR-IDs from malformed GDDs |
> | No architecture docs | HIGH | Cannot run `$ccgs-architecture-review` or `$ccgs-create-control-manifest` |
...
> |---|------|-------|----------|-------|
> | 1 | Create `design/gdd/systems-index.md` | `$ccgs-map-systems` or `$ccgs-adopt` inline | BLOCKING | Unblocks all downstream skills |
...
> | 5 | Create architecture ADRs from existing code | `$ccgs-reverse-document` + `$ccgs-architecture-decision` | HIGH | Captures decisions already made in code |
> | 6 | Create master architecture document | `$ccgs-create-architecture` | HIGH | After ADRs |
...
>
> I can create one now by reading your existing code and design docs to infer what systems you've built. This is faster than running `$ccgs-map-systems` from scratch — I'll populate it based on what already exists, then you confirm or adjust.
>
...
> - ✅ `$ccgs-gate-check` — now has a systems index to validate
> - ✅ `$ccgs-help` — can now show correct phase workflow
>
...
>
> After those 3, you'll have 5 of 7 systems with complete GDDs. Then run `$ccgs-review-all-gdds` to catch cross-system consistency issues before architecture work.
...
- **FORMAT audit, not existence audit**: `$ccgs-adopt` doesn't just check whether files exist — it checks whether their internal structure matches what skills expect. A file named `gdd.md` with no template sections is flagged as a gap.
...
- **Retrofit vs. new authoring**: `/design-system retrofit` handles existing-but-incomplete docs; full `$ccgs-design-system` handles net-new docs. The adoption plan distinguishes which applies to each file.
- **Context: fork**: `$ccgs-adopt` runs in a forked context to avoid polluting the main session with large file reads across the whole codebase.
