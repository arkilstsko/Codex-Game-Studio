...
- Ends with explicit handoff: "run `$ccgs-design-review` before the next system"

**Learn:**
- How `$ccgs-design-system` is different from asking an agent to "write a GDD"
...
**Type:** Full Workflow
**Skills:** `$ccgs-story-readiness` → implementation → `$ccgs-story-done`
**Duration:** ~50 minutes (13 turns)
**Complexity:** Medium

**Scenario:**
Dev picks up a story from the sprint backlog. `$ccgs-story-readiness` catches a roll-direction ambiguity before any code is written. After implementation, `$ccgs-story-done` verifies 9 acceptance criteria, identifies 2 deferred criteria (inventory not integrated yet), and closes the story with notes.

**Key Moments:**
- `$ccgs-story-readiness` catches spec ambiguity in Turn 2 — resolved before implementation starts
...
**Learn:**
- Why `$ccgs-story-readiness` prevents late-implementation ambiguity
...
**Type:** Phase Gate
**Skill:** `$ccgs-gate-check`
**Duration:** ~20 minutes (7 turns)
**Complexity:** Low

**Scenario:**
Dev completes the Systems Design phase and runs `$ccgs-gate-check` to advance. The gate finds all 6 MVP GDDs complete, cross-review passed with one low-severity concern. Gate passes, `stage.txt` updated, and the agent provides a specific ordered checklist for Technical Setup.
...
- stage.txt update changes what `$ccgs-help`, `$ccgs-sprint-status`, and all skills see going forward
...
**Skills:** `$ccgs-ux-design`, `$ccgs-ux-review`, `$ccgs-team-ui`
**Duration:** ~90 minutes (16 turns)
**Complexity:** Medium-High

**Scenario:**
Dev designs the HUD and inventory screen. `$ccgs-ux-design` reads the player journey and GDDs to ground decisions in player emotional state. `$ccgs-ux-review` catches a blocking accessibility gap (no keyboard alternative to drag-drop) and an advisory colorblind issue. After fixes, `$ccgs-team-ui` accepts the handoff.

**Key Moments:**
- HUD philosophy choice (diegetic vs. persistent vs. tactical) grounded in survival genre conventions
- `$ccgs-ux-review` distinguishes BLOCKING (stops handoff) vs. ADVISORY (can fix in visual pass)
- Accessibility caught before implementation, not during QA
- Keyboard alternative added in one turn; review re-runs and passes
- `$ccgs-team-ui` checks for a passing `$ccgs-ux-review` before starting visual design

**Learn:**
- How `$ccgs-ux-design` uses player journey context to ground UI decisions
- What `$ccgs-ux-review` actually checks (not just "does a spec exist?")
...
**Scenario:**
Dev has 3 months of existing code and rough design notes but nothing in the right format. `$ccgs-adopt` audits format compliance (not just file existence), classifies 4 gaps by severity, builds an ordered 7-step migration plan, and immediately fixes the BLOCKING gap (missing systems index) by inferring it from the codebase.
...
**Learn:**
- The difference between `$ccgs-adopt` and `$ccgs-project-stage-detect`
...
- **Agent Roster:** [codex-studio/docs/agent-roster.md](../../codex-studio/docs/agent-roster.md)
